<?php
// FILE: tasks-article.php - Writing Books/Articles
...
// 2. Fetch Article/Writing Tasks
$stmt = $pdo->prepare("
    SELECT t.*, ws.admin_status
    FROM tasks t
    LEFT JOIN worker_submissions ws ON t.id = ws.task_id AND ws.worker_id = ?
    WHERE t.status = 'Active' AND t.task_type IN ('Article Writing', 'Book Drafting', 'Content Creation') 
    ORDER BY t.payment_rwf DESC
");
$stmt->execute([$user_id]);
$tasks = $stmt->fetchAll(PDO::FETCH_ASSOC);

print_dashboard_header($pdo, "Earn by Writing");
print_sidebar();
?>
<div class="main-content">
    <div class="container-fluid">
        <h2 class="mb-4">✍️ Writing and Content Creation Tasks</h2>
            ...
            <?php if ($tasks): ?>
                <?php foreach ($tasks as $task): 
                    ...
                    $status_info = match($status_int) {
                        ...
                        default => ['Available', 'primary', 'Submit your finished work link.'],
                    };
                ?>
                    <div class="card mb-3 shadow-sm border-<?= $status_info[1]; ?>">
                        <div class="card-body text-dark">
                            ...
                            <p class="card-text">
                                ...
                                **Instructions:** <?= nl2br(htmlspecialchars($task['description'])); ?>
                            </p>
                            ...
                            <?php if ($status_int < 1): ?>
                                <button class="btn btn-primary btn-sm mt-2" data-bs-toggle="modal" data-bs-target="#submitModal<?= $task_id; ?>">
                                    <i class="fas fa-edit"></i> Submit Article Link
                                </button>
                            <?php else: ?>
                            ...
                            <div class="modal-content">
                                <form method="POST" action="submit_task.php">
                                    ...
                                    <div class="modal-body text-dark">
                                        ...
                                        <p>Please provide the link to your completed article/book draft (e.g., Google Docs, Dropbox).</p>
                                        <div class="mb-3">
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <div class="alert alert-info">No active writing tasks are currently available.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
